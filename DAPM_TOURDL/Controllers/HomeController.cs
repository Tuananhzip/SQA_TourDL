...
namespace DAPM_TOURDL.Controllers
{
    ...
    {
        private readonly QLTOUR db = new QLTOUR();
        ...
        {
            if (TempData["Mes"] != null) ViewBag.Mes = TempData["Mes"];
            return View();
        }
        ...
        public ActionResult DangKy(KHACHHANG khachhang)
        {
            ...
            var ngaySinh16 = ngayHienTai.AddYears(-16);
            if (db.KHACHHANGs.Any(x => x.Mail_KH == khachhang.Mail_KH))
            {
                ModelState.AddModelError("", "Tài khoản đã tồn tại");
            }
            else if (string.IsNullOrEmpty(khachhang.Mail_KH) || khachhang.GioiTinh_KH == null ||
                     khachhang.NgaySinh_KH == null || string.IsNullOrEmpty(khachhang.MatKhau) ||
                     string.IsNullOrEmpty(khachhang.CCCD) || string.IsNullOrEmpty(khachhang.SDT_KH) ||
                     string.IsNullOrEmpty(khachhang.HoTen_KH))
            {
            ...
            else if (!MatKhauManh(khachhang.MatKhau))
            {
                ModelState.AddModelError("",
                    "Mật khẩu phải có ít nhất 8 ký tự, bao gồm ít nhất 1 số, 1 chữ thường, 1 chữ hoa, 1 ký tự đặc biệt");
            }
            else
            {
        ...
        public ActionResult DangNhap(KHACHHANG khachhang)
        {
            var kiemTraDangNhap = db.KHACHHANGs
                .Where(x => x.Mail_KH.Equals(khachhang.Mail_KH) && x.MatKhau.Equals(khachhang.MatKhau))
                .FirstOrDefault();

            if (kiemTraDangNhap != null)
            {
                Session["UsernameSS"] = kiemTraDangNhap.HoTen_KH;
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ChinhSuaThongTinCaNhan(
            [Bind(Include = "ID_KH,HoTen_KH,GioiTinh_KH,NgaySinh_KH,MatKhau,CCCD,SDT_KH,Mail_KH,Diem")]
            KHACHHANG khachhang)
        {
            var ngayTruocKhiDu16Tuoi = DateTime.Now.AddYears(-16);
        ...
        public ActionResult LichSuDatTour(int id)
        {
            ...
            if (data.Count == 0) ViewBag.Mes = "Bạn chưa đặt tour nào cả";
            return View(data);
        }
        ...
        public ActionResult UrlPayment(int? orderCode)
        {
            ...
            var vnp_TmnCode =
                ConfigurationManager.AppSettings["vnp_TmnCode"]; //Ma định danh merchant kết nối (Terminal Id)
            var vnp_HashSecret = ConfigurationManager.AppSettings["vnp_HashSecret"]; //Secret Key

            //Build URL for VNPAY
            var vnpay = new VnPayLibrary();
            var price = order.TienPhaiTra;
        ...
        public ActionResult PaymentConfirm()
        {
            ...
            {
                var hashSecret = ConfigurationManager.AppSettings["vnp_HashSecret"]; //Chuỗi bí mật
                ...
                var orderId = Convert.ToInt64(pay.GetResponseData("vnp_TxnRef")); //mã hóa đơn
                var vnpayTranId =
                    Convert.ToInt64(pay.GetResponseData("vnp_TransactionNo")); //mã giao dịch tại hệ thống VNPAY
                var vnp_ResponseCode =
                    pay.GetResponseData("vnp_ResponseCode"); //response code: 00 - thành công, khác 00
                var vnp_SecureHash = Request.QueryString["vnp_SecureHash"]; //hash của dữ liệu trả về
                var vnp_TransactionStatus = pay.GetResponseData("vnp_TransactionStatus");
                var vnp_Amount = Convert.ToInt64(pay.GetResponseData("vnp_Amount")) / 100;
                /*                long vnp_Amount = Convert.ToInt64(pay.GetResponseData("vnp_Amount"));
                */
                var checkSignature = pay.ValidateSignature(vnp_SecureHash, hashSecret);
                if (checkSignature)
                {
                    if (vnp_ResponseCode == "00" && vnp_TransactionStatus == "00")
                    {
                        ...
                        ViewBag.Message = "Thanh toán thành công hóa đơn " + orderId + " | Mã giao dịch: " +
                                          vnpayTranId;
                        ViewBag.ThanhToanThanhCong = "Số tiền thanh toán (VND): " + string.Format("{0:C0}", vnp_Amount);
                    }
                    else
                    {
        ...
        public ActionResult DatTour(FormCollection form, string id, string paymentMethod)
        {
            ...
            if (Session["IDUser"] == null) return View(sptour);

            var currentIDUser = (int)Session["IDUser"];
            ...
            var slnguoilon = int.Parse(form["songuoilon"]);
            var sltreem = int.Parse(form["sotreem"]);
            var giaguoilon = (int)sptour.GiaNguoiLon;
            var giatreem = (int)sptour.GiaTreEm;
            /*int giaguoilon = int.Parse(form["gianguoilon"]);
                int giatreem = int.Parse(form["giatreem"]);*/

            //so sánh ...
            var soluong = slnguoilon + sltreem;
            var SoLuongSPTOUR = (int)sptour.SoNguoi;
            ...
            var tongtien = slnguoilon * giaguoilon + sltreem * giatreem;
            var VAT = (int)(tongtien * 0.05);
            ...
            db.SaveChanges();
            if (paymentMethod == "cashBtn") return RedirectToAction("HoaDon", "Home", new { id = hOADON.ID_HoaDon });
        ...
        public ActionResult DanhMucTour(string name, int? to, int? from, int page = 1)
        {
            ...
            if (!string.IsNullOrEmpty(name))
            {
                ...
                if (to != null) tours = tours.Where(x => x.DiemDen.Contains(name) && x.GiaNguoiLon >= to);
                if (from != null) tours = tours.Where(x => x.DiemDen.Contains(name) && x.GiaNguoiLon <= from);
            }
            else
            {
                if (to != null || from != null)
                {
                    ...
                    else if (from != null) tours = tours.Where(x => x.GiaNguoiLon <= from);
                }
            }
            ...
            if (tours.Count() == 0) ViewBag.MesSearch = "Không tìm thấy tour nào với lựa chọn của bạn !";
